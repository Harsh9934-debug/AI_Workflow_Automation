...
export const Hero: React.FC<HeroProps> = ({ title, description, children }) => {
    return (
        <section className="relative h-screen w-screen font-sans tracking-tight text-gray-900 bg-white overflow-hidden">
            ...
            {/* Content Container */}
            {children && (
                <div className="absolute inset-0 z-20 flex items-center justify-start pointer-events-none pl-4 md:pl-20">
            ...
            {(title || description) && (
                <div className="absolute bottom-4 left-4 md:bottom-10 md:left-10 z-20 max-w-md pointer-events-none">
                    {title && <h1 className="text-3xl font-light tracking-tight mb-3">
                        {title}
                    </h1>}
                    {description && <p className="text-gray-700 text-sm leading-relaxed font-light tracking-tight">
                        {description}
                    </p>}
                </div>
            )}
